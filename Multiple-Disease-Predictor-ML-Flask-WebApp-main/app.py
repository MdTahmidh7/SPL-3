...
heart_predict=pickle.load(open('random_forest_model.pkl', 'rb'))
...
def predictheartdisease():
    # Get the data as JSON and extract the form values
    print(request)
    int_features = [float(x) for x in request.json.values()]  # Directly accessing JSON data
    ...
    # Prepare the result
    if prediction[0] == 1:
        display_text = "This person has risk of Heart Disease"
    else:
        display_text = "This person doesn't have Heart Disease"
    
    # Return the result as JSON
    return jsonify({'output_text': display_text})
